...
def main():
    ...
    experiment_ls = ['clean',  'cutout/exp_resnet_cutout_patch_size_60.0','cutout/exp_resnet_cutout_patch_size_100.0', 'cutout/exp_resnet_cutout_patch_size_120.0']
    ...
    if 'all' not in args.experiment:
        for epoch in range(0, 90, 6):
            print(f"Processing epoch {epoch}")
            extractor = LayerActivityExtractor(
                checkpoint_path=f'{output_path}/checkpoints/epoch{epoch}.pth',
                image_folder=args.data_path,
                batch_size=args.batch_size, 
                num_workers=args.num_workers,
            ...
            X_train, X_test = split_activity(activities, save_path=f'{result_path}/activities_pca_epoch{epoch}.npz', test_size=0)
            network_names = activities.keys()

            distmat = compute_distances(X_train, X_train)
            os.makedirs(f'{output_path}/epochs_{epoch}', exist_ok=True)
            np.save(f'{output_path}/epochs_{epoch}/distance_matrix_epochs_{epoch}.npy', distmat)
            visualize_distance(distmat, network_names, f'{output_path}/epochs_{epoch}')

            del X_train, X_test, activities, extractor
